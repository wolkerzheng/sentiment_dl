...
if __name__ == "__main__":
    ...
    output.to_csv("xgb_word2vec.csv", index=False, quoting=3)

    # test = pd.read_csv(os.path.join(os.path.dirname(__file__), 'xgb_word2vec.csv'), header=0,
    #                    quoting=3)
    # result = []
    # for val in test['sentiment']:
    #     if val - 0.5 > 0.000000001:
    #         result.append(1)
    #     else:
    #         result.append(0)
    # output = pd.DataFrame(data={"id": test['id'], "sentiment": result})
    # output.to_csv(os.path.join(os.path.dirname(__file__), 'data', 'xgb_tfidf_model.csv'),
